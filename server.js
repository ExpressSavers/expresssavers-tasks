...
result = await db.execute(` SELECT m.*, u.name as sender_name, u.color as sender_color, r.name as recipient_name FROM messages m JOIN users u ON m.sender_id = u.id LEFT JOIN users r ON m.recipient_id = r.id ORDER BY m.created_at DESC LIMIT 100`);
} else {
result = await db.execute(` SELECT m.*, u.name as sender_name, u.color as sender_color, r.name as recipient_name FROM messages m JOIN users u ON m.sender_id = u.id LEFT JOIN users r ON m.recipient_id = r.id WHERE m.recipient_id = ? OR m.recipient_id = 'ALL' OR m.recipient_site = ? OR m.recipient_site = 'ALL' OR m.sender_id = ? ORDER BY m.created_at DESC LIMIT 50`,
[userId, req.user.site, userId]);
}
// Count unread
const unread = await db.execute(
`SELECT COUNT(*) as cnt FROM messages  WHERE (recipient_id=? OR recipient_id='ALL' OR recipient_site=? OR recipient_site='ALL') AND sender_id != ? AND read_at IS NULL`,
[userId, req.user.site, userId]);
...
await db.execute(
`INSERT INTO messages (id, sender_id, recipient_id, recipient_site, subject, body, priority, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
...
await db.execute(
`INSERT INTO messages (id, sender_id, recipient_id, recipient_site, subject, body, parent_id, priority, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'reply', datetime('now'))`,
...
const result = await db.execute(` SELECT ta.*, t.name as task_name, t.shift, u.name as user_name, u.color as user_color FROM task_assignments ta JOIN tasks t ON ta.task_id = t.id JOIN users u ON ta.user_id = u.id WHERE ta.site = ? AND (ta.date = ? OR ta.permanent = 1) ORDER BY t.shift, t.sort_order`,
...
await db.execute(
`INSERT INTO task_assignments (id, task_id, user_id, site, date, permanent, note, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
...
const daily = await db.execute(`SELECT  c.date, c.site, COUNT(DISTINCT CASE WHEN c.is_done=1 THEN c.task_id END) as done, COUNT(DISTINCT CASE WHEN c.penalty_triggered=1 AND c.penalty_waived=0 THEN c.id END) as penalties, SUM(CASE WHEN c.penalty_triggered=1 AND c.penalty_waived=0 THEN t.penalty_amount ELSE 0 END) as penalty_total, COUNT(DISTINCT u.id) as staff_count FROM completions c  JOIN users u ON c.user_id=u.id JOIN tasks t ON c.task_id=t.id WHERE c.date >= date('now','-'||?||' days') GROUP BY c.date, c.site  ORDER BY c.date DESC`, [days]);
// Top missed tasks
const missed = await db.execute(`SELECT t.name, t.site, t.shift, COUNT(*) as miss_count FROM completions c JOIN tasks t ON c.task_id=t.id WHERE c.is_done=0 AND c.date >= date('now','-'||?||' days') GROUP BY c.task_id ORDER BY miss_count DESC LIMIT 10`, [days]);
// Best performing staff
const topStaff = await db.execute(`SELECT u.name, u.site, u.color, COUNT(CASE WHEN c.is_done=1 THEN 1 END) as tasks_done, COUNT(CASE WHEN c.penalty_triggered=1 AND c.penalty_waived=0 THEN 1 END) as penalties FROM users u LEFT JOIN completions c ON u.id=c.user_id AND c.date >= date('now','-'||?||' days') WHERE u.is_manager=0 GROUP BY u.id ORDER BY tasks_done DESC`, [days]);
res.json({
