...
if(useCloudinary) console.log('OK Cloudinary configured -- photos will be stored permanently in cloud');
else console.warn('!?  Cloudinary not configured -- photos stored locally (will be lost on redeploy)');
...
async function initDB() {
  ...
  if (!dbUrl || dbUrl === 'file:./tasks.db') {
    console.warn('!?  WARNING: Using local database file. Data will be lost on redeploy.');
    console.warn('!?  Set DB_URL environment variable to a Turso database URL for persistent storage.');
    console.warn('!?  See DEPLOYMENT_GUIDE.txt for instructions.');
  } else {
    console.log('OK Using external database:', dbUrl.split('?')[0]);
  }
